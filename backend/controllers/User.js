...
exports.updateProfile = async (req, res) => {
  try {
    ...
    const avatarFromBody =
      typeof req.body.avatar === 'string' && req.body.avatar.startsWith('data:image/')
        ? req.body.avatar
        : null;
    const avatarSource = req.file?.path || avatarFromBody;
...
exports.firebaseGoogleAuth = async (req, res) => {
  try {
    ...
    if (!email) {
      return res.status(400).json({
        success: false,
        message: 'Facebook account did not provide an email address. Please use a Facebook account with an email or sign in with email/password.'
      });
    }
